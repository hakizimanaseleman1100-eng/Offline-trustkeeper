...
// SHA-256 of `${business_id}:${pin}` as lowercase hex. Both login and staff
// creation MUST hash through here so the stored hash and the login hash match.
// The raw PIN never leaves this function.
export async function hashPin(pin) {
  const data = new TextEncoder().encode(`${CURRENT_BUSINESS_ID}:${pin}`);

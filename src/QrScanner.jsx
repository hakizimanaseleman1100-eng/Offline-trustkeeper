...
// Camera QR scanner using the native BarcodeDetector (Chrome / Android — the
// target devices). Calls onResult with the decoded text once; the parent closes
// it. Used by the barman to receive a waiter's round at the counter.
//
// The manual-paste fallback is not decoration: if the camera fails at 9pm the
// bar cannot stop serving, and the alternative is the barman keying the whole
// round in by hand.
function QrScanner({ onResult, onClose, title = 'Scan the waiter’s round', hint }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!('BarcodeDetector' in window)) {
        setError('This device can’t scan QR codes. Use “Enter code manually”.');
        return;
      }
      try {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-slate-900 flex flex-col items-center justify-center gap-4 p-6">
        ...
        <textarea
          ...
          placeholder="Paste the code shown under the waiter's QR…"
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center">
      ...
      {hint && <p className="absolute top-16 inset-x-6 text-center text-white/70 text-sm">{hint}</p>}
      <button
        onClick={() => setManual('')}
